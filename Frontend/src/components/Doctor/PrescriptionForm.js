...
const PrescriptionForm = ({ selectedPrescription, patientMail }) => {
  ...
  const validate = () => {
    ...
    // Age Validation
    tempErrors.age = formValues.age && formValues.age > 0 ? "" : "Valid age is required.";
    ...
    if (validate()) {
      try {
        const url = selectedPrescription
          ? `http://localhost:5000/prescriptions/prescriptions/${selectedPrescription._id}`
          : 'http://localhost:5000/prescriptions/prescriptions';
        ...
        if (response.status === 201 || response.status === 200) {
          await axios.post(`http://localhost:5000/prescriptions/mailSend`, { email: patientMail, rx: formValues.rx });
        }
        ...
        if (response.ok) {
          await Swal.fire({
            title: "Success!",
            text: `Prescription successfully ${selectedPrescription ? 'updated' : 'submitted'}.`,
            icon: 'success',
      ...
      <form onSubmit={handleSubmit}>
        <Grid container spacing={2}>
          ...
          <Grid item xs={12} sm={6} sx={{ display: 'flex', justifyContent: 'space-between' }}>
            <div></div>
            <Button variant="contained" color="error" onClick={handleClear} sx={{ height: '50px' }}>
              Clear
            </Button>
            <Button type="submit" variant="contained" color="secondary" sx={{ height: '50px' }}>
