...
const Prescriptions = () => {
    ...
    const fetchPrescriptions = async () => {
        try {
            const response = await fetch('http://localhost:5000/prescriptions/prescriptions');
    ...
    const filteredPatients = prescriptions.filter(prescription =>
        ...
        (genderFilter ? prescription.gender === genderFilter : true) && (prescription.email === doctor.email)
    );
    ...
    const handleDelete = async (id) => {
        try {
            await fetch(`http://localhost:5000/prescriptions/prescriptions/${id}`, {
                method: 'DELETE',
            });
            fetchPrescriptions();
            handleClose();
                ...
                <Grid container spacing={2} style={{ marginTop: '20px' }}>
                    {filteredPatients.map(prescription => (
                        <Grid item xs={12} md={6} key={prescription._id}>
                            ...
                            >
                                <CardContent>
                                    <Typography variant="subtitle1" color='secondary' fontWeight='bold'>{prescription.firstName} {prescription.lastName}</Typography>
                                    <Typography variant="body2" fontWeight='bold'>Age: {prescription.age}</Typography>
                                    <Typography variant="body2" fontWeight='bold'>Date: {prescription.date}</Typography>
                                    <Typography variant="body2" fontWeight='bold'>Mail: {prescription.patientEmail}</Typography>
                                </CardContent>
                                <ArrowForwardIosIcon />
                            </Card>
                        </Grid>
                ...
                {selectedPrescription && (
                    <Dialog open={open} onClose={handleClose} maxWidth="xs" fullWidth>
                        ...
                        <DialogActions sx={{ justifyContent: 'center', paddingBottom: '20px' }}>
                            ...
                            <Button variant="contained" onClick={() => handleDelete(selectedPrescription._id)}>Delete</Button>
                            <Button variant="contained" color="error" onClick={handlePrint}>Print</Button>
                        </DialogActions>
                    </Dialog>
                )}
